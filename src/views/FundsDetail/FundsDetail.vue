<template>
  <div class="funds-wrapper display-flex flex-column headview_wrapper">
    <ims-header title="资金明细">
      <div slot="right"
           class="header-cottage"
           @click="onRightClick"></div>
    </ims-header>
    <div class="tab-container">
      <div class="tab-content display-flex">
        <div class="tab-item flex-1 display-flex flex-center"
             :class="{ 'active': activeTab == 0 }"
             @click="chooseTab(0)">收入</div>
        <div class="tab-item flex-1 display-flex flex-center"
             :class="{ 'active': activeTab == 1 }"
             @click="chooseTab(1)">支出</div>
      </div>
    </div>
    <div class="main-container flex-1 display-flex flex-column">
      <fundsComp :activeTab="activeTab"
                 :dateType="dateType"></fundsComp>
    </div>
    <div class="bottom-popup-container">
      <van-popup v-model="switchs.showPopup"
                 position="bottom">
        <div v-for="(v,k) in popList"
             :key="k"
             @click="popClick(v,k)"
             class="popup-item ims--txt--midum">{{ v.name }}</div>
        <div class="popup-item cacle-txt"
             @click="switchs.showPopup = false">取消</div>
      </van-popup>
    </div>
  </div>
</template>
